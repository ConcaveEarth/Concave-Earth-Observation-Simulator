...
function renderProfileScaleGuide(
  ...
  return (
    <g key={`${panel.sceneKey}-profile-scale`}>
      <line
        x1={start.x}
        y1={start.y}
        x2={end.x}
      ...
      {Array.from({ length: 5 }, (_, index) => {
        ...
        return (
          <g key={`${panel.sceneKey}-profile-scale-${index}`}>
            <line
              x1={x}
              y1={start.y - 7}
              x2={x}
...
export function ProfileVisibilityView({
  panels,
  compareLayout,
  unitPreferences,
  language,
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id="profileBackdrop" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#07131f" />
          <stop offset="55%" stopColor="#0a1f30" />
          <stop offset="100%" stopColor="#111827" />
        </linearGradient>
        <linearGradient id="profilePanelFill" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="rgba(9, 24, 38, 0.88)" />
          <stop offset="100%" stopColor="rgba(9, 18, 30, 0.72)" />
        </linearGradient>
        <linearGradient id="profileSurfaceFill" x1="0" y1="0" x2="0" y2="1">
        ...
        {panels.map((panel, index) => (
          <clipPath key={panel.sceneKey} id={`profile-clip-${panel.sceneKey}`}>
            <rect
      ...
      <rect width={svgWidth} height={svgHeight} fill="url(#profileBackdrop)" rx={30} />
        ...
        return (
          <g key={panel.sceneKey}>
            <rect
              ...
              height={rect.height}
              rx={28}
              fill="url(#profilePanelFill)"
              stroke="rgba(141, 192, 255, 0.18)"
            />

            <g clipPath={`url(#profile-clip-${panel.sceneKey})`}>
              <polygon
                points={polygonPoints(surfacePolygon)}
                fill="url(#profileSurfaceFill)"
                opacity={0.96}
              />
              ...
              {panel.traces.map((trace) => (
                <polyline
                  ...
                  filter={!trace.dashed ? "url(#profileGlow)" : undefined}
                />
              ))}

              {panel.profileSegments.map((segment) => (
                <polyline
                  key={segment.id}
                  points={polygonPoints(segment.points.map(project))}
                  fill="none"
              ...
              {panel.samplePoints.map((sample) => {
                ...
                return (
                  <circle
                    ...
                    opacity={sample.visible ? 0.98 : 0.48}
                  />
                );
              })}
